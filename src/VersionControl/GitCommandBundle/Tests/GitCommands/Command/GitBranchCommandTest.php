...
class GitBranchCommandTest extends GitCommandTestCase{
    ...
    public function testCreateLocalBranch(){
        ...
        $this->assertEquals("Switched to branch 'branch2'", $branchCommand->createLocalBranch('branch2',true), 'create branch command and switch');
        ...
        $this->assertEquals("fatal: A branch named 'branch1' already exists.", trim($errorMessage), 'create branch command error');
    }
    ...
    public function testGetBranches(){
        ...
        $branchCommand->createLocalBranch('branch1');
        $this->assertEquals(array("branch1","master"), $branchCommand->getBranches(), ' branch listing does not equal expected');

        $this->assertEquals(array("branch1","master"), $branchCommand->getBranches(true), ' branch listing does not equal expected');
    }
    ...
    public function testDeleteBranch(){
        ...
        }catch(\VersionControl\GitCommandBundle\GitCommands\Exception\DeleteBranchException $e){
            $errorMessage = $e->getMessage();
        }
        $this->assertEquals("You cannot delete the current branch. Please checkout a different branch before deleting.", trim($errorMessage), 'create branch command error');
