...
class IssueController extends BaseProjectController
{
    ...
    public function searchAjaxAction($id, Request $request)
    {
        ...
        foreach ($issueEntities as $issueEntity) {
            $result[] = array(
                'id' => $issueEntity->getId(), 'title' => $issueEntity->getTitle(), 'description' => $issueEntity->getDescription(), 'status' => $issueEntity->getStatus(), 'author' => $issueEntity->getUser()->getName(),
            );
        }
