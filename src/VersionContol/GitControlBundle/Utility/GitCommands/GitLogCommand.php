...
/**
 */
class GitLogCommand extends GitCommand {
    ...
    public function getCommand(){
        ...
        $this->command = 'git --no-pager log -m "--pretty=format:\''.$this->format.'\'"';
        if($this->logCount){
            $this->command .= ' -'.intval($this->logCount).' ';
        }
        
        if($this->showReferences === true){
            $this->command .= ' --decorate ';
        }
        ...
        if($this->filterByDateAfter){
           $this->command .= ' --after='.escapeshellarg($this->filterByDateAfter);
        }
        
        if($this->filterByDateBefore){
           $this->command .= ' --before='.escapeshellarg($this->filterByDateBefore);
        }
        
        if($this->filterByAuthor){
           $this->command .= ' --author='.escapeshellarg($this->filterByAuthor);
        }
    ...
    public function execute(){
        ...
        try{
            ...
            $this->logData = $this->runCommand($command);
