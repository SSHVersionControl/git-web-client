...
class GitDiffCommand extends GitCommand {
    ...
    public function getDiffFileBetweenCommits($filename,$previousCommitHash,$commitHash){
         $diffString = $this->runCommand("git --no-pager diff  --oneline ".escapeshellarg($previousCommitHash)." ".escapeshellarg($commitHash)." ".escapeshellarg($filename)." 2>&1");
         $diffParser = new GitDiffParser($diffString);
         $diffs = $diffParser->parse(); 
         return $diffs;
    }
    ...
    public function getPreviousCommitHash($commitHash = 'HEAD'){
        $previousCommitHash = '';
        $response = $this->runCommand(" git log --pretty=format:'%h' -n 2 ".escapeshellarg($commitHash)."");
        $responseLines = $this->splitOnNewLine($response);
